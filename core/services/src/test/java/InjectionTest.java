...
public class InjectionTest {
    ...
    @BeforeAll
    public void init(){
        given(productRepositoryPort.findById(any())).willReturn(Optional.ofNullable(prepareTv()));
        given(orderRepositoryPort.findById(any())).willReturn(Optional.ofNullable(prepareOrder()));
        given(userRepositoryPort.findById(any())).willReturn(Optional.ofNullable(prepareAdmin()));

        given(productRepositoryPort.findAll()).willReturn(List.of(prepareTv(), prepareTv()));
        given(orderRepositoryPort.findAll()).willReturn(List.of(prepareOrder(), prepareOrder()));
        given(userRepositoryPort.findAll()).willReturn(List.of(prepareAdmin(), prepareAdmin()));
    }
